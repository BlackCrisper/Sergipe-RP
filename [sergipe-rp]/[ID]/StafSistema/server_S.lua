...
-- /dv ID   Destruir o Veiculo da pessoa
-- /fix ID  Arruma o veiculo da pessoa
-- /vida ID da vida para a pessoa
-- /colete ID  da colete para a pessoa
-- /setgas ID  Enche o tank da pessoa
-- /fly   O Admin Pode voar
-- /v     O Admin fica invisible
-- /ss    Para altera sua skin
-- /setfome ID   Para coloca Fome/Sede para a Pessoa
-- /tp ID para ir ate o jogador
-- /puxar ID  para trazer o jogador ate vc

-- Lembrando Necessario ta na  ACL ( Staff ) 
    ...
    if(id) then
		...
		if(playerID) then
				...
				if (isPedInVehicle(targetPlayer)) then
					...
					setTimer(setElementAngularVelocity, 50, 10, veh, 0, 0, 0)
					setElementInterior(veh, interior)
					setElementDimension(veh, dimension)	
				else
