...
function createHouse(hdata)
	local enterMarker = createMarker(hdata.enterMrkX, hdata.enterMrkY, hdata.enterMrkZ - 0.3, 'corona', 1.3, 0,153,0, 100)
	setElementData(enterMarker, "houseData", {ID = hdata.ID, cost = hdata.cost, owner = hdata.owner} )
	if (hdata.owner ~= "") then
		setMarkerColor(enterMarker, 255, 51, 36, 100)
	end
	
	local exitMarker = createMarker(hdata.exitMrkX, hdata.exitMrkY, hdata.exitMrkZ - 0.3, 'corona', 1.3, 255,255,255, 100)
...
function updateHouse(ID, key, newValue)
	if (key == "owner") then
		...
		if (newValue ~= "") then
			setMarkerColor(markersByID[ID], 255, 51, 36, 100)
		else
			setMarkerColor(markersByID[ID], 0,153,0, 100)
		end
	end
end
...
function enterMarkerHit(player, matchingDimension)
	if (player == localPlayer) and (houseEnterMarkers[source]) and (not isPedInVehicle(player)) and (matchingDimension) then
		if myAccount then
			...
			--outputChatBad("Por favor entre!")
			exports.YM_DxMessages:addBox( "Por favor entre!", "info")	
		end
	end
end
...
function outputChatOK(text)
	--outputChatBox("[Gestão da casa] #33FF24"..text, 56,120,172, true)
	exports.YM_DxMessages:addBox( ""..text.."", "info")	
end

function outputChatBad(text)
	--outputChatBox("[Gestão da casa] #FF3324"..text, 56,120,172, true)
	exports.YM_DxMessages:addBox( ""..text.."", "info")	
end
...
-- ==========     Divida os números em partes     ==========
function explodeNumber(number)
    number = tostring(number)
    local k
    repeat
        number, k = string.gsub(number, "^(-?%d+)(%d%d%d)", '%1 %2')
    until (k==0)    -- true - sair do loop
    return number
end
