...
addEventHandler("onClientResourceStart", resourceRoot, function()
		-- ================= JANELA DE CRIAÇÃO DA CASA =================
		GUI.window.creation = guiCreateWindow((screenW-360), (screenH-500)/2, 330, 500, "Construindo uma casa", false)
		guiWindowSetSizable(GUI.window.creation, false)

		GUI.label[1] = guiCreateLabel(10, 25, 150, 15, "Marcador de entrada", false, GUI.window.creation)
		...
		GUI.button.setEnterMrkPos = guiCreateButton(10, 155, 150, 35, "Definir coordenadas", false, GUI.window.creation)

		GUI.label[2] = guiCreateLabel(170, 25, 150, 15, "Marcador de saída", false, GUI.window.creation)
		...
		GUI.button.setExitMrkPos = guiCreateButton(170, 155, 150, 35, "Definir coordenadas ", false, GUI.window.creation)

		GUI.label[3] = guiCreateLabel(10, 210, 150, 15, "Coordenadas de login", false, GUI.window.creation)
		...
		GUI.button.setEnterPosPos = guiCreateButton(10, 340, 105, 35, "Definir coordenadas", false, GUI.window.creation)
        GUI.button.tpEnter = guiCreateButton(125, 340, 35, 35, "ТП", false, GUI.window.creation)

		GUI.label[4] = guiCreateLabel(170, 210, 150, 15, "Coordenadas de saída", false, GUI.window.creation)
		...
		GUI.button.setExitPosPos = guiCreateButton(170, 340, 105, 35, "Definir coordenadas", false, GUI.window.creation)
        GUI.button.tpExit = guiCreateButton(285, 340, 35, 35, "ТП", false, GUI.window.creation)

		GUI.label[5] = guiCreateLabel(10, 395, 58, 25, "Interior", false, GUI.window.creation)
		...
		GUI.label[6] = guiCreateLabel(170, 395, 30, 25, "Preço", false, GUI.window.creation)
        ...
        GUI.label[1] = guiCreateLabel(140, 434, 115, 15, "Slots de transporte", false, GUI.window.creation)
        guiSetFont(GUI.label[1], "default-bold-small")
        GUI.edit.carLots = guiCreateEdit(265, 430, 55, 25, "", false, GUI.window.creation)
        guiEditSetReadOnly(GUI.edit.carLots, false)   

		GUI.button.interiorPanel = guiCreateButton(10, 455, 80, 35, "<<< Painel interior", false, GUI.window.creation)

		GUI.button.create = guiCreateButton(100, 465, 80, 25, "Criar uma", false, GUI.window.creation)

		GUI.button.clear = guiCreateButton(205, 465, 80, 25, "Claro", false, GUI.window.creation)

		GUI.button.closeCreation = guiCreateButton(295, 465, 25, 25, "X", false, GUI.window.creation)

		-- ================= JANELA INTERIOR =================
		GUI.window.interiors = guiCreateWindow((screenW-900), (screenH-500)/2, 540, 410, "Seleção de interiores", false)
		...
		guiGridListAddColumn(GUI.gridlist.interiors, "Nome", 0.87)

		GUI.staticimage.interior = guiCreateStaticImage(230, 25, 300, 300, "images/choose.jpg", false, GUI.window.interiors)

		GUI.button.saveReturnPos = guiCreateButton(10, 335, 80, 65, "Lembre-se da posição de retorno", false, GUI.window.interiors)
		GUI.button.moveToInterior = guiCreateButton(100, 335, 95, 65, "Mover para o interior", false, GUI.window.interiors)
		GUI.label.returnToWorld = guiCreateLabel(205, 335, 235, 20, "Posição de retorno não salva", false, GUI.window.interiors)
		guiLabelSetVerticalAlign(GUI.label.returnToWorld, "center")
		GUI.button.returnToWorld = guiCreateButton(205, 365, 80, 35, "Retornar", false, GUI.window.interiors)

		GUI.button.moveCoordinates = guiCreateButton(450, 335, 80, 65, "Transferir para\ndados\n===>>>", false, GUI.window.interiors)
...
function onAdminGUIClick()
	...
	elseif (source == GUI.button.tpEnter) then
		...
		if (not x) or (not y) or (not z) or (not int) then
			outputChatBad("Os 3 campos acima e o interior devem ser preenchidos!")
			return
		end
	...
	elseif (source == GUI.button.tpExit) then
		...
		if (not x) or (not y) or (not z) then
			--outputChatBad("3 campos de coordenadas devem ser preenchidos!")
			exports.YM_DxMessages:addBox( "3 campos de coordenadas devem ser preenchidos!", "warning")	
			return
		end
	...
	elseif (source == GUI.button.interiorPanel) or (source == GUI.button.closeInteriors) then
		if not guiGetVisible(GUI.window.interiors) then
			guiSetText(GUI.button.interiorPanel, ">>> Painel interior")
	...
	elseif (source == GUI.button.create) then
		...
		if (newData.enterMrkX == "") or (newData.enterMrkY == "") or (newData.enterMrkZ == "")
			...
			--outputChatBad("Nem todos os campos estão preenchidos!")
			exports.YM_DxMessages:addBox( "Nem todos os campos estão preenchidos!", "warning")	
	...
	elseif (source == GUI.button.saveReturnPos) then
		...
		guiSetText(GUI.label.returnToWorld, "Envio sobre: "..math.round(savedPosX, 3)..", "..math.round(savedPosY, 3)..", "..math.round(savedPosZ, 3))
	...
	elseif (source == GUI.button.moveToInterior) then
		...
		if (selected ~= -1) then
			...
			--outputChatBad("Interior não selecionado! ")
			exports.YM_DxMessages:addBox( "Interior não selecionado!", "info")	
		end
	...
	elseif (source == GUI.button.returnToWorld) then
		if savedPosX then
			...
			--outputChatBad("Nenhuma posição salva!")
			exports.YM_DxMessages:addBox( "Nenhuma posição salva!", "warning")	
		end
	...
	elseif (source == GUI.button.moveCoordinates) then
		...
		if (selected ~= -1) then
			...
			--outputChatBad("Interior não selecionado!")
			exports.YM_DxMessages:addBox( "Interior não selecionado!", "warning")	
		end
	...
	if (source == GUI.edit.price) then
		...
		if (price) then
			if (price < 200000) then
				guiSetText(GUI.edit.carLots, 1)
			elseif (price < 500000) then
				guiSetText(GUI.edit.carLots, 2)
			elseif (price < 1200000) then
				guiSetText(GUI.edit.carLots, 3)
			elseif (price < 2000000) then
				guiSetText(GUI.edit.carLots, 4)
			elseif (price < 4000000) then
				guiSetText(GUI.edit.carLots, 6)
			elseif (price < 10000000) then
				guiSetText(GUI.edit.carLots, 6)
			elseif (price < 20000000) then
				guiSetText(GUI.edit.carLots, 100)
			elseif (price < 30000000) then
				guiSetText(GUI.edit.carLots, 200)
			elseif (price < 50000000) then										
				guiSetText(GUI.edit.carLots, 500000)
			else
...
function drawCrsrInfo()
	if isCursorShowing() and not cursorOverGUI then
		...
		if x and y then
			dxDrawText("M para ocultar / mostrar o cursor.", screenW * x + 10, screenH * y - 10, screenW, screenH, white, 1, 'default', 'left', 'top', false, false, true)
		end
	end
end
