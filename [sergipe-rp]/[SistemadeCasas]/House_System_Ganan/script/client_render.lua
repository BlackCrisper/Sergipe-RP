...
function refreshMyHouseBlips()
	...
	for _, marker in ipairs( getElementsByType("marker", resourceRoot) ) do
		...
		if (data) and (data.owner == myAccount) then
			myHouseBlips[data.ID] = createBlipAttachedTo(marker, 21, 0, 0,0,0,255,0, 500)
		end
	end
end
			...
			if (dist > 3) and (dist < 30) then
				...
				if px then
					if isLineOfSightClear(cX, cY, cZ, x, y, z, true, true, false, true, false, false, false, marker) then
						...
						local cost = explodePrice(houseData.cost)
						local r,g,b = getMarkerColor(marker)
						if (owner == "") then owner = "n/a." end
						if (owner == myAccount) then r,g,b = 56, 120, 172 end
						dxDrawText("Proprietário: "..owner,		px+1,	py+screenH/38,	px+1,	py+1,	black, 1, 'default-bold', 'center', 'center', false, false )
						dxDrawText("Proprietário: "..owner,		px,		py+screenH/38,	px,		py,		white, 1, 'default-bold', 'center', 'center', false, false )
						dxDrawText("Preço: "..cost.." R$.",	px+1,	py+screenH/15,	px+1,	py+1,	black, 1, 'default-bold', 'center', 'center', false, false )
						dxDrawText("Preço: "..cost.." R$.",	px,		py+screenH/15,	px,		py,		white, 1, 'default-bold', 'center', 'center', false, false )
						dxDrawImage(px-pictX/2, py-screenH/12, pictX, pictY, "images/house_icon.png", 0, 0, 0, tocolor(r,g,b, 255) );
					end
				end
			end
		end
...
bindKey("h", "both", function(_, keyState)
