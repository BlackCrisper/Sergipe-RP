...
function dxGridW:AddItem ( columnIndex, text, data, r, g, b )
    ...
    if __checkParams ( "AddItem", "ns", columnIndex, text ) then
        if self.i[columnIndex] then	
            local tColor = __checkRGB ( r, g, b ) and { r, g, b } or { 0, 30, 121,180 };
        ...
        if #dxGridW.items > 0 then
            ...
            for index, data in ipairs ( dxGridW.items ) do
                ...
                if data.vis then
                    ...
                    dxDrawRectangle ( data.x, data.y, data.w, data.h, tocolor ( 0, 0, 0, 0 ), data.pg );

                    -- Draw the column bar
                    dxDrawRectangle ( data.x, data.y, data.w, 30 % data.h, tocolor ( 0, 25, 101, 0 ), data.pg );
                    ...
                    if data.se and data.se <= data.r and data.se >= seeFrom and data.se <= seeTo then
                        ...
                        dxDrawRectangle ( data.x, ( 30 % data.h ) + y2, data.w, 27, tocolor ( 0, 0, 0, 50 ), data.pg );
                    ...
                    local cWidth = 0
                        ...
                        for cIndex, cData in ipairs ( data.i ) do
                            ...
                            if ( ( cWidth + cData.info.width ) <= data.w ) then
                                ...
                                dxDrawText ( cData.info.title, x, data.y, cData.info.width + x, ( 30 % data.h ) + data.y, tocolor ( 0, 30, 121,181 ), FIT_MODE and ( 1 * SCALE ) or 1, "default-bold", "center", "center", true, false, data.pg, true, false );
                                    ...
                                    for iIndex = seeFrom, seeTo do
                                        ...
                                        if cData[iIndex] then
                                         ...
                                         --   dxDrawImage( x+150, y+15, 900, 2, "linea.png", 0,0,0 ,tocolor(255,255,255,105))
                                            ...
                                            dxDrawText ( cData[iIndex]["text"], x, y, cData.info.width + x, ( 35 % data.h ) + y, tocolor ( unpack ( cData[iIndex]["color"] ) ), FIT_MODE and ( 0.8 * SCALE ) or 0.8, "default-bold", "center", "center", false, false, data.pg, true, false );
                                            end
                                        end
                                    end
                                end
