...
local font = dxCreateFont("font/icons.ttf", 12, false, "proof")
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("TS:PolicialPack")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("TS:PolicialPack")[editBoxId])) then
				if (getElementData(getElementsByType("TS:PolicialPack")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("TS:PolicialPack")[editBoxId], "state", true)
				end 
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("TS:PolicialPack")) do
		if (getElementData(element, "state") == true) then
