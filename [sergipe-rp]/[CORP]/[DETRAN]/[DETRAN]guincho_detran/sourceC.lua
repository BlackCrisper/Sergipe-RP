	...
	function ()
											...
											if tx then
												if i == 4 then
													if holder == localPlayer then
														outputChatBox("O cabo não pode passar aqui!", 255, 255, 255, true)
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				outputChatBox("O cabo não chega tão longe!", 255, 0, 0, true)
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								outputChatBox("Puxe a rampa primeiro!", 255, 255, 255, true)
							end
						end
					end
				end
						...
						if not rampInAnim[veh] then
							if key == "cardown" then
								if getElementData(veh, "packerState") == "down" then
									...
									outputChatBox("Solte a rampa primeiro!", 255, 255, 255, true)
								end
									...
									if getVehicleType(clickedElement) == "Automobile" then
										...
										if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 5 then
											...
											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												...
												outputChatBox("O carro não está atrás do porta-carros!", 255, 255, 255, true)
											end
										else
											outputChatBox("Você está muito longe do veículo!", 255, 255, 255, true)
										end
									else
										outputChatBox("Você só pode reservar carros[.", 255, 255, 255, true)
									end
								end
							end
						end
