...
local font = dxCreateFont("font/icons.ttf", 10, false, "proof")
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("TS:EditBoxArmas")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("TS:EditBoxArmas")[editBoxId])) then
				if (getElementData(getElementsByType("TS:EditBoxArmas")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("TS:EditBoxArmas")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("TS:EditBoxArmas")) do
		if (getElementData(element, "state") == true) then
