...
local Marker_Assalto1 = createMarker(2731.89, -2074.723, 13.554 -1, "cylinder", 1.2, 255, 0, 255, 0)
...
function Iniciar_Assalto1 (source)
	...
	local Valor_Ganho = math.random(100, 3000) -------------------------------------/ VALOR GANHO
	local Procurado = getPlayerWantedLevel(source)
	if isElementWithinMarker(source, Marker_Assalto1) then
		if getElementData(source, "Zeed_Assaltando3") then
		...
		if getElementData(source, "Zeed_Loja_Assaltada3") then
		...
		setElementData(source, "Zeed_Assaltando3", true)
		...
		setTimer(function()
    		...
    		setElementData(source, "Zeed_Assaltando3", false)
    		setElementData(source, "Zeed_Loja_Assaltada3", true)
...
function Loja_Assaltada (source)
	...
	setElementVisibleTo(Blip_Assaltada, root, true)
        triggerClientEvent(root, "N3xT.dxNotification", root, "[ATENÇÃO] o jogador '."..getPlayerName(source)..".' esta saqueando a lojinha proximo ao Estadio, em Los Santos.", "info")
        setTimer(function()
        setElementVisibleTo(Blip_Loja, root, true)
        setElementVisibleTo(Blip_Assaltada, root, false)
        setElementData(source, "Zeed_Loja_Assaltada3", false)
